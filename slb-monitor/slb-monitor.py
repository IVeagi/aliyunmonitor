...
credentials = AccessKeyCredential('xxxxxxxx', 'xxxxxxx')
client = AcsClient(region_id='cn-beijing', credential=credentials)

p = Producer({'bootstrap.servers':'1.1.1.1:9092,2.2.2.2:9092,3.3.3.3:9092'})
p.poll(0)
    ...
    for j in range(len(slb_metrics)):
        ...
        try:
            ...
            for k in range(len(slb_data)):
                ...
                data_analyze = ("kafka {0} SLB {1} {2} {3} {4} {5} {6} {7} wtf").format(end_time, instance_id, instance_name, instance_vip, instance_port, metric_name, metric_desc,average_value)
